...
this.modal = new Modal({
  caption : 'グループ追加',
  content : new Convert(this.template_group).double_blancket(data),
  buttons : [
    {
      label : 'cancel',
      click : (()=>{this.modal.close()}).bind(this),
    },
    {
      label : 'ok',
      click : (()=>{
        const elm_group = document.querySelector(`.modal_window .modal_content [name='group_name']`)
        if(!elm_group.value){
        ...
        new Ajax({
          ...
          query : {
            mode: 'post_group_add',
            group_name : elm_group.value,
          },
          success : this.add_group_posted.bind(this)
        })
      }).bind(this),
    },
  ],
